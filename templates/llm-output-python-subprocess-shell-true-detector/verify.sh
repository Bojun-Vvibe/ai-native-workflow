#!/usr/bin/env bash
# verify.sh — worked-example end-to-end check for shell=True detector.
set -u
...
bad_rc=$?
bad_count="$(printf '%s\n' "$bad_out" | grep -cE ': (subprocess-shell-true-interp|os-system-interp|os-popen-interp|commands-getoutput-interp):')"

good_out="$(python3 detect.py examples/good 2>&1)"
good_rc=$?
good_count="$(printf '%s\n' "$good_out" | grep -cE ': (subprocess-shell-true-interp|os-system-interp|os-popen-interp|commands-getoutput-interp):')"
