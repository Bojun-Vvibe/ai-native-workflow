#!/usr/bin/env bash
# verify.sh — worked-example end-to-end check for nodejs TLS verify-disabled detector.
set -u
...
bad_out="$(node detect.js examples/bad 2>&1)"
bad_rc=$?
bad_count="$(printf '%s\n' "$bad_out" | grep -cE ': (tls-rejectunauthorized-false|tls-env-disable|tls-globalagent-disable)')"

good_out="$(node detect.js examples/good 2>&1)"
good_rc=$?
good_count="$(printf '%s\n' "$good_out" | grep -cE ': (tls-rejectunauthorized-false|tls-env-disable|tls-globalagent-disable)')"
