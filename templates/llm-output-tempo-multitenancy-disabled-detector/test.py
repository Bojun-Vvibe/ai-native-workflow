#!/usr/bin/env python3
"""Runnable harness for the tempo-multitenancy-disabled detector.

Iterates ``examples/bad/`` (must each return True from ``detect``)
and ``examples/good/`` (must each return False). Prints a single
...
def main() -> int:
    ...
    if bad_hits == len(bad_files) and good_fps == 0:
        ...
        return 0
    print(
        f"FAIL bad={bad_hits}/{len(bad_files)} good_fp={good_fps}/{len(good_files)}"
    )
    return 1
