#!/usr/bin/env bash
# verify.sh — worked-example end-to-end check for CORS-wildcard detector.
# Asserts: bad/ flags >= MIN_BAD, good/ flags == 0. Exits 0 on success.
set -u
...
bad_rc=$?
bad_count="$(printf '%s\n' "$bad_out" | grep -cE ': (flask-cors-|flask-cross-origin-|manual-allow-origin-|django-cors-|starlette-cors-)')"

good_out="$(python3 detect.py examples/good 2>&1)"
good_rc=$?
good_count="$(printf '%s\n' "$good_out" | grep -cE ': (flask-cors-|flask-cross-origin-|manual-allow-origin-|django-cors-|starlette-cors-)')"
