...
BAD_TOTAL=$(find examples/bad -type f -name '*.py' | wc -l | tr -d ' ')
GOOD_TOTAL=$(find examples/good -type f -name '*.py' | wc -l | tr -d ' ')

bad_hits=0
for f in examples/bad/*.py; do
