#!/usr/bin/env bash
# verify.sh — worked-example check for os.system / os.popen user-input detector.
set -u
...
bad_rc=$?
bad_count="$(printf '%s\n' "$bad_out" | grep -cE ': (os-system-fstring|os-system-concat|os-system-percent|os-system-format)')"

good_out="$(python3 detect.py examples/good 2>&1)"
good_rc=$?
good_count="$(printf '%s\n' "$good_out" | grep -cE ': (os-system-fstring|os-system-concat|os-system-percent|os-system-format)')"
