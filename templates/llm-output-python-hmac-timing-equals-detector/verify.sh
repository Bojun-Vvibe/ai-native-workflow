#!/usr/bin/env bash
# verify.sh — worked-example end-to-end check for HMAC timing-equals detector.
set -u
HERE="$(cd "$(dirname "$0")" && pwd)"
cd "$HERE"

MIN_BAD=5  # 5 bad files, at least 5 findings (one file has 2)

bad_out="$(python3 detect.py examples/bad 2>&1)"
bad_rc=$?
bad_count="$(printf '%s\n' "$bad_out" | grep -cE ': timing-unsafe-(==|!=)-on-(secret|digest):')"

good_out="$(python3 detect.py examples/good 2>&1)"
good_rc=$?
good_count="$(printf '%s\n' "$good_out" | grep -cE ': timing-unsafe-(==|!=)-on-(secret|digest):')"
