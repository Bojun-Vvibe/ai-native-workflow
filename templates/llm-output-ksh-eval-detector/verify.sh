#!/usr/bin/env bash
# verify.sh — worked-example end-to-end check for ksh-eval detector.
...
bad_rc=$?
bad_count="$(printf '%s\n' "$bad_out" | grep -c ': ksh-eval ')"

good_out="$(python3 detect.py examples/good 2>&1)"
good_rc=$?
good_count="$(printf '%s\n' "$good_out" | grep -c ': ksh-eval ')"
...
if [ "$fail" -eq 0 ]; then
  echo "OK: ksh-eval detector verified (bad=$bad_count, good=$good_count)"
  exit 0
fi
exit 1
