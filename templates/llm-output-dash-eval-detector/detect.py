#!/usr/bin/env python3
"""Detect dangerous dash `eval` invocations on dynamic data.

dash (Debian Almquist Shell, /bin/sh on Debian/Ubuntu/Alpine) is a
small, strictly POSIX shell. Its `eval` builtin concatenates its
...
Recurses directories looking for *.dash files and any file whose
first line is a `#!.../dash` shebang (or `#!/bin/sh` — dash is the
default on many distros, audit those too).
"""
...
def is_dash_file(path: Path) -> bool:
    if path.suffix == ".dash":
        return True
    try:
    ...
    if not first.startswith("#!"):
        return False
    # Match `dash`, and `/bin/sh` (which is dash on Debian/Ubuntu).
    return "dash" in first or first.rstrip().endswith("/sh")
    ...
    for idx, raw in enumerate(text.splitlines(), start=1):
        ...
        for m in RE_EVAL.finditer(scrub):
            ...
            while tok_pos < len(scrub) and scrub[tok_pos].isspace():
                tok_pos += 1
            col = tok_pos + 1
            findings.append((path, idx, col, "dash-eval-dynamic", raw.strip()))
    return findings
    ...
    for r in roots:
        ...
        if p.is_dir():
            for sub in sorted(p.rglob("*")):
                if sub.is_file() and is_dash_file(sub):
                    yield sub
        elif p.is_file():
            yield p
