#!/usr/bin/env bash
# verify.sh — worked-example end-to-end check for the Terraform
# AWS S3 public-ACL / public-access-block detector.
set -u
...
KIND_RE=': (tf-s3-acl-public|tf-s3-pab-disabled|tf-s3-pab-missing|tf-s3-policy-principal-wildcard|tf-s3-website-public|tf-s3-cors-allow-all-origins):'
