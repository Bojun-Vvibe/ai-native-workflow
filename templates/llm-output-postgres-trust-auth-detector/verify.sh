...
BAD_TOTAL=$(find examples/bad -type f -name 'pg_hba.conf' | wc -l | tr -d ' ')
GOOD_TOTAL=$(find examples/good -type f -name 'pg_hba.conf' | wc -l | tr -d ' ')

bad_hits=0
for f in examples/bad/*/pg_hba.conf; do
...
good_hits=0
for f in examples/good/*/pg_hba.conf; do
  python3 detector.py "$f" >/dev/null 2>&1
  rc=$?
