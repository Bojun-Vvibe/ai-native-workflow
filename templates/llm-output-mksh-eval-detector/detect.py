#!/usr/bin/env python3
"""Detect dangerous mksh `eval` invocations on dynamic data.
...
including `eval` (`command eval ...`) to bypass functions named
`eval`. We flag that form too.
...
* `command eval $var`         (mksh `command` prefix bypass form)
...
Recurses directories looking for *.mksh files and any file whose
first line is a `#!.../mksh` shebang.
"""
...
def is_mksh_file(path: Path) -> bool:
    if path.suffix == ".mksh":
        return True
    try:
    ...
    for idx, raw in enumerate(text.splitlines(), start=1):
        ...
        for m in RE_EVAL.finditer(scrub):
            ...
            while tok_pos < len(scrub) and scrub[tok_pos].isspace():
                tok_pos += 1
            col = tok_pos + 1
            findings.append((path, idx, col, "mksh-eval-dynamic", raw.strip()))
    return findings
    ...
    for r in roots:
        ...
        if p.is_dir():
            for sub in sorted(p.rglob("*")):
                if sub.is_file() and is_mksh_file(sub):
                    yield sub
        elif p.is_file():
            yield p
