#!/usr/bin/env bash
# verify.sh — worked-example end-to-end check for scala-akka-actor-blocking-call detector.
set -u
...
bad_rc=$?
bad_count="$(printf '%s\n' "$bad_out" | grep -cE ': akka-actor-blocking-')"

good_out="$(python3 detect.py examples/good 2>&1)"
good_rc=$?
good_count="$(printf '%s\n' "$good_out" | grep -cE ': akka-actor-blocking-')"
