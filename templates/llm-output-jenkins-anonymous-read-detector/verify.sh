...
BAD_TOTAL=$(find examples/bad -type f -name 'config.xml' | wc -l | tr -d ' ')
GOOD_TOTAL=$(find examples/good -type f -name 'config.xml' | wc -l | tr -d ' ')

bad_hits=0
for f in examples/bad/*/config.xml; do
...
good_hits=0
for f in examples/good/*/config.xml; do
  python3 detector.py "$f" >/dev/null 2>&1
  rc=$?
